<template>
  <article 
    ...
    <NuxtLink :to="`/proyectos/${project.slug}`">
      <NuxtImg :src="project.picture ?? ''" class="w-full rounded-t aspect-video object-cover" :alt="project.name" />
    </NuxtLink>
    <div class="p-5">
      <NuxtLink :to="`/proyectos/${project.slug}`">
      ...
      <p class="mb-3 font-normal text-gray-600 dark:text-gray-300">
        <small class="text-gray-500 dark:text-gray-400 block">
          By {{ project.user?.name }} | {{ project.created_at }}
        </small>
        {{ project.description }}
      </p>
    </div>
